...
from derby_darts.forms.model_forms import (
    TeamForm,
    PlayerForm,
    FixtureForm,
    LeagueForm,
...
def validate_number(request):
    ...
    return JsonResponse(
        data={
            "valid": Player.objects.filter(
                phone_number=number, team__name=team_name
            ).exists()
        }
    )
...
class TeamListView(ListView):
    ...
    def get_queryset(self):
        current_season = (
            Season.objects.filter(league=self.request.league)
            .order_by("-start_date")
            .first()
        )
        return Team.objects.filter(
            league=self.request.league, seasonstanding__season=current_season
        ).order_by("name")
    ...
    def form_valid(self, form):
        ...
        for player in players:
            Player.objects.get_or_create(
                team=team, name=player.lstrip("@"), is_county="@" in player
            )
...
class PostListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        can_edit = (
            ...
            and self.request.user.privileges.can_create_wall_posts
        )
        if can_edit:
...
def index(request):
    ...
    can_edit = (
        ...
        and request.user.privileges.can_create_wall_posts
    )
...
def login_user(request):
    logout(
        request
    )  # Only needed to handle users that come to login page by url when logged in
    notification = None
    ...
    if request.POST:
        ...
        if user is None:
            notification = (
                "error",
                "The username and password you have provided is incorrect.",
            )
    ...
    return render(
        ...
        {"form": AuthenticationForm(), "notification": notification},
    )
...
def new_competition_fixture(request):
    return render_form(
        request, "New Competition Fixture", CompetitionFixtureForm(request=request)
    )
...
def new_result(request):
    ...
    result_form.fields["fixture"].queryset = Fixture.objects.filter(
...
class ResultFormView(FormView):
    ...
    def form_valid(self, form):
        form.save()
        return JsonResponse(
            status=200, data={"message": "{0} added".format(str(form.instance))}
        )
...
def render_form(request, form_title, form):
    ...
    return render(request, "base_form.html", {"form_title": form_title, "form": form})
...
def set_standings(request):
    ...
    return render(
        request,
        "set_standings.html",
        {"teams": Team.objects.filter(league=request_league), "seasons": seasons},
    )
...
def post_standings(request):
    ...
    season = Season.objects.get(league=request.league, id=request.POST["season"])

    for team_dict in team_list:
        team_name = team_dict["team_name"]
        section = team_dict["section"]

        team = Team.objects.get(league=request.league, name=team_name)
        ...
        if existing:
            ...
            standing = SeasonStanding(season=season, team=team, section=section)
            standing.save()
...
class ReceiveMsgView(View):
    def handle_invalid_msg_format(self, from_number, text, date):
        ...
        msg = UnprocessedMessage.objects.create(
            text=text, date=prsd_date, sender=from_number
        )
        ...
        send_mail(
            subject="Invalid Result Text Message",
            message=msg,
    ...
    def post(self, request):
        ...
        missing_result_fixtures = Fixture.objects.filter(
        ...
        if len(parsed_msg) == 3:
            # Expected format of won/lost score only
            ...
            # Give fuzzy matching a chance
            fuzzy_result = self.attempt_fuzzy_result(latest_fixture, text)
            if fuzzy_result is None:
                return self.handle_invalid_msg_format(from_number, text, date)
            result_dict = dict(result_dict, **fuzzy_result)

        result = Result.objects.create(**result_dict)

        return JsonResponse(status=200, data={"message": str(result)})

    def attempt_fuzzy_result(fixture, text_message):
        teams = fixture.home_team, fixture.away_team
        found_teams = []
        for team in teams:
            match = find_near_matches(team, text_message, max_l_dist=3)
            if match:
                found_teams.append((team, match[0].start))
                text_message = text_message.replace(
                    text_message[match[0].start : match[0].end], ""
                )

        found_teams = [team[0] for team in sorted(found_teams, key=lambda x: x[1])]
        if len(found_teams) == 2:
            scores = re.findall(r"(\d+)", text_message)
            if len(scores) == 2:
                result = {team: score for team, score in zip(found_teams, scores)}
                return {
                    "home_team_score": result[teams[0]],
                    "away_team_score": result[teams[1]],
                }
        return None
