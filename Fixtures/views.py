...
from derby_darts.models import (
    Fixture,
    Result,
    Season,
    Team,
...
def existing_fixtures(request):
    ...
    current_season = (
        Season.objects.filter(league=request.league).order_by("-start_date").first()
    )
    fixtures = Fixture.objects.filter(season=current_season, date=p_date)
    serializer = FixtureSerializer(fixtures, many=True)
    test_data = CompetitionFixture.objects.select_related("fixture").filter(
        season=current_season, date=p_date
    )
    fixture_data = [
        [fixture.get_home_team_name(), fixture.get_away_team_name()]
    ...
    if request.method == "POST":
        ...
        for team in Team.objects.filter(league=request.league):
            Fixture.objects.create(
                competition=competition, season=season, date=date, home_team=team
            )
...
class UpdateResultsView(ListView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(
            season__league=self.request.league,
            result__isnull=True,
            date__lte=datetime.now(),
        )
...
def set_results(request):
    no_results = Fixture.objects.filter(
        season__league=request.league, result__isnull=True, date__lte=datetime.now()
    )
...
def set_score(request):
    ...
    if request.REQUEST:
        fixture = Fixture.objects.get(
            season__league=request.league, id=request.REQUEST["id"]
        )
        ...
        result = Result(
            fixture=fixture, home_team_score=home_score, away_team_score=away_score
        )
        result.save()
...
def __render_team_fixtures_for_season(request, season):
    seasons = Season.objects.filter(league=request.league).values("name", "id")
    sections = list(
        season.seasonstanding_set.values(
            "season_section__name", "season_section__id"
        ).distinct()
    )

    return render(
        request,
        "fixtures_for_team.html",
        {"sections": sections, "current_season": season, "all_seasons": seasons},
    )
...
def latest_sections(request, season_id=None):
    ...
    sections = season.seasonstanding_set.distinct("section").values(
        "season_section", "season_section__name", "season_section__order"
    )
...
def __teams_for_section_response(season, section_id):
    ...
    teams = [standing.team.name for standing in section_standings]
...
def __fixtures_for_team_response(request, season, team_name):
    ...
    fixtures = Fixture.objects.filter(
        Q(home_team=team)
        | Q(away_team=team)
        | Q(competitionfixture__isnull=False, home_team__isnull=True),
        season=season,
    ).order_by("date")
    fixtures = list(
        fixtures.values(
...
def import_fixtures(request):
    if request.method == "POST":
        ...
        if season is not None:
            season = Season.objects.get(id=int(season), league=request.league)
            ...
            return render(
                request,
                "import.html",
                {"teams": teams, "season": season, "sections": list(range(sections))},
            )
    return render(
        request,
        "import_select_season.html",
        {"seasons": Season.objects.filter(league=request.league)},
    )
...
def import_season(request, season_id):
    if request.method == "POST":
        ...
        importer = CSVImporter(
            StringIO(request.FILES["file_upload"].read().decode("utf-8"))
        )
        ...
        dates = [
            datetime.strptime(date_string, "%d/%m/%y") if date_string else date_string
            for date_string in date_strings
        ]
        sections = {
            k.split("_")[1]: request.POST.getlist(k)
        ...
        for section, team_ids in sections.items():
            ...
            season_section, created = Section.objects.get_or_create(
                league=request.league, name=section, order=section_num
            )
            teams = [
                Team.objects.get(league=request.league, id=int(team_id))
            ...
            for row_idx, row in enumerate(fixtures):
                ...
                for idx, fixture in enumerate(row):
                    if fixture.endswith("H") and len(fixture) < 4:
                        ...
                        team_fixtures.append((dates[idx], home_team, away_team))
    ...
    return render(request, "import.html", {"teams": teams, "sections": 1})
...
class SwapTeamView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {"seasons": seasons, "teams": teams})
    ...
    def post(self, request, *args, **kwargs):
        ...
        team_a_standing = SeasonStanding.objects.filter(
            season=season, team_id=int(team_a_id)
        )
        ...
        team_a_fixtures = Fixture.objects.filter(
            Q(home_team=team_a) | Q(away_team=team_a), season=season
        )

        team_b_standing = SeasonStanding.objects.filter(
            season=season, team_id=int(team_b_id)
        )
        if not team_b_standing.exists():
            return JsonResponse(status=101, data={"value": "Team A not in this season"})
        team_b_standing = team_b_standing[0]
        team_b = team_b_standing.team
        team_b_fixtures = Fixture.objects.filter(
            Q(home_team=team_b) | Q(away_team=team_b), season=season
        )
        ...
        return JsonResponse(
            status=200,
            data={"value": "{} swapped with {}".format(team_a.name, team_b.name)},
        )
...
class FixtureMakerView(FormView):
    ...
    def form_valid(self, form):
        ...
        return render(
            self.request, "forms/created_fixtures.html", {"fixtures": fixtures}
        )
